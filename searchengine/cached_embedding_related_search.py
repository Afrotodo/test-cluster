...
Usage in typesense_calculations.py:
    ...
    # After search completes, store the query + embedding + result count
    store_query_embedding(query, query_embedding, result_count=len(results))
    
    # Get related searches for sidebar (only returns queries with results)
    related = get_related_searches(query_embedding, limit=5, exclude_query=query)

Redis Storage:
    qcache:meta:{hash}  - Query text, timestamp, search count, result count
...
# Result count threshold - queries must have at least this many results to be shown
MIN_RESULT_COUNT = config('MIN_RESULT_COUNT', default=1, cast=int)
...
def store_query_embedding(
    ...
    Does NOT store document UUIDs - only the query, embedding, and result count.
    ...
    Args:
        ...
        result_count: How many results the query returned (used to filter out zero-result queries)
    ...
    try:
        # Check if already exists - if so, just increment count and update result_count
        ...
        if existing:
            ...
            # Update result_count if the new value is higher (in case results were added)
            # or if the stored value was 0 (in case this is the first time we're tracking it)
            current_result_count = redis_client.hget(meta_key, 'result_count')
            if current_result_count is None or int(current_result_count) < result_count:
                pipe.hset(meta_key, 'result_count', result_count)
        ...
        # Store metadata (including result_count)
        ...
        logger.debug(f"Stored new query: {normalized[:50]}... (result_count={result_count})")
        return True
...
def get_related_searches(
    ...
    Args:
        ...
        min_results: Minimum result count required (default: 1, filters out zero-result queries)
        
    Returns:
        List of dicts: [{'query': str, 'similarity': float, 'search_count': int, 'result_count': int}, ...]
    """
    if not embedding:
        return []
    ...
    try:
        ...
        for query_hash in all_hashes:
            ...
            # Get metadata first to check result_count before computing similarity
            meta_key = _get_key('meta', query_hash)
            ...
            # Skip queries with insufficient results
            ...
            if min_similarity <= similarity <= max_similarity:
                similarities.append((query_hash, similarity, meta))
        ...
        related = []
        for query_hash, similarity, meta in similarities[:limit]:
            related.append({
...
def get_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'queries_zero_results': total_zero_results,
...
def cleanup_zero_result_queries() -> int:
    """
    Remove queries that have zero results.
    Useful for cleaning up stale queries that no longer return results.
    ...
    try:
        ...
        for query_hash in all_hashes:
            meta_key = _get_key('meta', query_hash)
            result_count = redis_client.hget(meta_key, 'result_count')
            
            if result_count is None or int(result_count) == 0:
                _delete_query(query_hash)
                deleted += 1
        
        if deleted:
            logger.info(f"Cleanup: deleted {deleted} zero-result queries")
        return deleted
        
    except RedisError as e:
        logger.error(f"Zero-result cleanup failed: {e}")
        return 0
...
def health_check() -> Dict[str, Any]:
    ...
    if redis_client:
        try:
            ...
            index_key = f"{CACHE_KEY_PREFIX}:index"
            all_hashes = redis_client.zrange(index_key, 0, -1)
            result['queries_stored'] = len(all_hashes) if all_hashes else 0
            
            # Count queries with results
            queries_with_results = 0
            for query_hash in (all_hashes or []):
                meta_key = _get_key('meta', query_hash)
                result_count = redis_client.hget(meta_key, 'result_count')
                if result_count and int(result_count) > 0:
                    queries_with_results += 1
            result['queries_with_results'] = queries_with_results
...
def bootstrap_from_analytics(
    ...
    Args:
        ...
        result_count_func: Optional function to get result count for a query (returns int)
    ...
    Usage:
        from typesense_calculations import get_query_embedding, get_result_count
        from cached_embedding_related_search import bootstrap_from_analytics
        
        stats = bootstrap_from_analytics(get_query_embedding, get_result_count, limit=500)
    """
    redis_client = _get_redis_client()
    if not redis_client:
    ...
    try:
        ...
        for query, count in popular:
            ...
            try:
                ...
                # Get result count if function provided
                result_count = 0
                if result_count_func:
                    try:
                        result_count = result_count_func(query)
                    except Exception:
                        pass  # Default to 0 if we can't get result count
                
                # Store it
                if store_query_embedding(query, embedding, search_count=int(count), result_count=result_count):
                    stats['stored'] += 1
                else:
                    stats['failed'] += 1
...
def debug_related_searches(
    ...
    try:
        ...
        print(f"   Min results required: {min_results}")
        print(f"{'-'*60}")
        ...
        for query_hash in all_hashes:
            ...
            result_count = int(meta.get('result_count', 0)) if meta else 0
            ...
            if result_count < min_results:
                excluded_no_results.append((stored_query, result_count))
                continue
            ...
            if similarity >= min_similarity and similarity <= max_similarity:
                included.append((stored_query, similarity, result_count))
            elif similarity > max_similarity:
                excluded_too_similar.append((stored_query, similarity, result_count))
            else:
                excluded_too_different.append((stored_query, similarity, result_count))
        ...
        if excluded_no_results:
            print(f"\n🚫 FILTERED (no results, <{min_results}):")
            for q, rc in excluded_no_results:
                print(f"      • {q[:50]} → {rc} results")
        
        if excluded_too_similar:
            print(f"\n⚠️  TOO SIMILAR (>{max_similarity}):")
            for q, sim, rc in sorted(excluded_too_similar, key=lambda x: -x[1]):
                print(f"      • {q[:50]} → sim={sim:.3f}, results={rc}")
        
        if excluded_too_different:
            print(f"\n❌ TOO DIFFERENT (<{min_similarity}):")
            for q, sim, rc in sorted(excluded_too_different, key=lambda x: -x[1]):
                print(f"      • {q[:50]} → sim={sim:.3f}, results={rc}")
        
        if included:
            print(f"\n✅ INCLUDED ({min_similarity}-{max_similarity}):")
            for q, sim, rc in sorted(included, key=lambda x: -x[1]):
                print(f"      • {q[:50]} → sim={sim:.3f}, results={rc}")
        else:
            print(f"\n✅ INCLUDED: None")
        ...
        print(f"   Filtered out: {len(excluded_no_results)} queries with no results")
        print(f"{'='*60}\n")
...
def get_popular_queries(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    Only returns queries that have at least MIN_RESULT_COUNT results.
    """
    redis_client = _get_redis_client()
    if not redis_client:
        return []
    ...
    try:
        ...
        for query_hash in all_hashes:
            ...
            if meta and meta.get('query'):
                result_count = int(meta.get('result_count', 0))
                # Only include queries that have results
