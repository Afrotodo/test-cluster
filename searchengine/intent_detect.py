...
def detect_intent(discovery_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    signals = {
        ...
        # ── context_flags-derived signals (v3 frozenset sync) ──────────
        # Pre-set by the context_flags reader below; existing logic may
        # also set these independently — both paths are additive.
        'has_food_word': False,
        'has_beauty_word': False,
        'has_entertainment_word': False,
        'has_culture_word': False,
        'has_color_modifier': False,
        'has_size_modifier': False,
        'has_acronym': False,
    }

    # =================================================================
    # PRE-SET SIGNALS FROM WORD DISCOVERY context_flags (v3 sync)
    # Runs BEFORE existing intent logic. Does not replace it.
    # Reads flags written by the WordDiscovery pre-gate in step 1 so
    # that words added to any WD frozenset automatically flow through
    # to intent detection — one source of truth.
    # =================================================================
    for term in terms:
        flags = term.get('context_flags', [])

        if 'known_food' in flags:
            signals['has_service_word'] = True
            signals['has_food_word'] = True

        if 'known_service' in flags:
            signals['has_service_word'] = True

        if 'known_beauty' in flags:
            signals['has_service_word'] = True
            signals['has_beauty_word'] = True

        if 'known_apparel' in flags:
            signals['has_product_word'] = True

        if 'known_entertainment' in flags:
            signals['has_entertainment_word'] = True

        if 'known_culture' in flags:
            signals['has_culture_word'] = True

        if 'known_superlative' in flags:
            signals['has_superlative'] = True

        if 'known_color' in flags:
            signals['has_color_modifier'] = True

        if 'known_size' in flags:
            signals['has_size_modifier'] = True

        if 'known_acronym' in flags:
            signals['has_acronym'] = True

    # ── Existing intent detection logic continues below ──────────────
...
def print_intent_debug(discovery_result: Dict[str, Any]) -> None:
    ...
    print(f"  has_beauty_word: {signals.get('has_beauty_word')}")
    print(f"  has_entertainment_word: {signals.get('has_entertainment_word')}")
    ...
    print(f"  has_verb: {signals.get('has_verb')}")
    print(f"  has_unknown_terms: {signals.get('has_unknown_terms')} ({signals.get('unknown_term_count')})")
    print(f"  has_color_modifier: {signals.get('has_color_modifier')}")
    print(f"  has_size_modifier: {signals.get('has_size_modifier')}")
    print(f"  has_acronym: {signals.get('has_acronym')}")
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        # context_flags test — weave from BEAUTY frozenset
        {
            'name': 'context_flags: weave from BEAUTY frozenset',
            'query': 'black weave',
            'terms': [
                {'word': 'black', 'search_word': 'black', 'pos': 'adjective', 'category': 'color', 'rank': 0, 'is_stopword': False, 'context_flags': ['known_color']},
                {'word': 'weave', 'search_word': 'weave', 'pos': 'noun', 'category': 'Keyword', 'rank': 800, 'is_stopword': False, 'context_flags': ['known_beauty']},
            ],
            'ngrams': [],
            'category_summary': {},
            'sort': None,
            ...
            'expected': {
                'has_service_word': True,
                'has_beauty_word': True,
                'has_color_modifier': True,
            },
        },
        # context_flags test — durag from APPAREL_PRODUCTS
        {
            'name': 'context_flags: durag from APPAREL_PRODUCTS frozenset',
            'query': 'durag',
            'terms': [
                {'word': 'durag', 'search_word': 'durag', 'pos': 'noun', 'category': 'Keyword', 'rank': 800, 'is_stopword': False, 'context_flags': ['known_apparel']},
            ],
            'ngrams': [],
            'category_summary': {},
            'sort': None,
