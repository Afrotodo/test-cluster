...
# LOCATION_INTENT_FLAGS = frozenset({
#     'known_service', 'known_food', 'known_beauty', 'known_apparel',
# })

# KEYWORD_INTENT_FLAGS = frozenset({
#     'known_color', 'known_superlative', 'known_size', 'known_modifier',
#     'known_culture', 'known_entertainment', 'known_acronym',
# })
...
#         # =================================================================
#         # COMMON VOCABULARY OVERRIDE
#         # Words that should ALWAYS be classified as keywords, never as
#         # city/person/organization entities. These are factual, stable
...
#         ALWAYS_KEYWORD = (
#             SUPERLATIVES | COLORS | SIZES | COMMON_ADJECTIVES |
#             APPAREL_PRODUCTS | FOOD_DINING | SERVICES
#         )
...
#             # COMMON VOCABULARY GATE: if word is in the always-keyword set,
#             # skip entity classification and go straight to keywords.
#             # Also applies POS gate for modifier words not in the set.
#             word_lower = wd['word'].lower()
#             predicted_pos = normalize_pos_string(wd.get('predicted_pos'))
#             match_pos = normalize_pos_string(sm.get('pos'))

#             if word_lower in ALWAYS_KEYWORD or (
#                 predicted_pos in MODIFIER_POS and match_pos in ('noun', 'proper_noun')
#                 and not is_pos_compatible(match_pos, predicted_pos)
#             ):
#                 keywords.append(entry)
#                 intent_scores['keyword'] += rank
#                 if self.verbose:
#                     if word_lower in ALWAYS_KEYWORD:
#                         print(f"  VOCAB GATE: '{word_lower}' → demoted to keyword (common vocabulary)")
#                     else:
#                         print(f"  POS GATE: '{word_lower}' predicted={predicted_pos}, match={match_pos}/{cat_lower} → demoted to keyword")
#                 continue
...
PIPELINE (6 Steps):
    ...
    Step 2: Context-Aware N-gram Resolution (rank comparison + signals)
    Step 3: POS Determination + Best Match Selection (merged from v2 3, 3.5, 4)
    Step 3.5: Pattern-Based "in" Disambiguation (NEW)
    Step 4: Batch Correct Unknowns (Redis fuzzy — only Redis step)
    Step 5: Re-check N-grams After Correction
    Step 6: Build Complete Profile (entities, locations, search terms, boosts)
...
PERFORMANCE:
    Steps 1-3, 3.5, 5-6: RAM only (~0.01ms per lookup)
    Step 4: Redis only for unknown words (~50ms per correction)
    Typical query (no typos): ~1-3ms
    Query with 1-2 typos: ~50-100ms
...
# =============================================================================
# STEP 3.5 DISAMBIGUATION CONSTANTS (NEW)
# =============================================================================

# Context flags from Step 1 that signal the LEFT side of "in" is a
# service/category — meaning the word after "in" should stay as a city.
LOCATION_INTENT_FLAGS = frozenset({
    'known_service', 'known_food', 'known_beauty', 'known_apparel',
})

# Context flags from Step 1 that signal the LEFT side of "in" is a
# descriptor/modifier/topic — meaning the word after "in" should be
# demoted from city to keyword.
KEYWORD_INTENT_FLAGS = frozenset({
    'known_color', 'known_superlative', 'known_size', 'known_modifier',
    'known_culture', 'known_entertainment', 'known_acronym',
})
...
class WordDiscovery:
    ...
    def process(self, query: str) -> Dict[str, Any]:
        ...
        ngrams, consumed_positions = self._step2_resolve_ngrams(words, word_data)
        self._step3_pos_and_select(word_data, consumed_positions)
        self._step3_5_resolve_in_ambiguity(word_data, consumed_positions)
        corrections = self._step4_correct_unknowns(word_data)
        corrected_words = self._get_working_words(word_data)
        new_ngrams, new_consumed = self._step5_recheck_ngrams(corrected_words, word_data, consumed_positions)
        ngrams.extend(new_ngrams)
    ...
    def _step3_5_resolve_in_ambiguity(self, word_data: List[Dict[str, Any]], consumed_positions: Set[int]) -> None:
        """
        Step 3.5: Pattern-Based 'in' Disambiguation.

        When a word after "in" matches BOTH the city hash AND the vocabulary,
        inspect the left-side context to decide whether it's a location or keyword.

        Rules:
            Left has service/food/category flag → LOCATION (keep city match)
            Left has descriptor/adj + noun      → KEYWORD  (demote city match)
            Word after "in" is city-ONLY        → LOCATION (always)
            No clear context                    → LOCATION (conservative default)

        Performance: O(n) scan, negligible cost (~0.01ms).
        """
        if self.verbose:
            print("\n" + "-" * 70)
            print("🎯 STEP 3.5: Pattern-Based 'in' Disambiguation")
            print("-" * 70)
        ...
        for i, wd in enumerate(word_data):
            # Find "in" stopwords
            if not (wd['is_stopword'] and wd['word'] == 'in'):
                continue

            # Need a word after "in"
            if i + 1 >= len(word_data):
                continue

            target = word_data[i + 1]

            # Skip if target is consumed by n-gram, stopword, or unknown
            if target['position'] in consumed_positions:
                continue
            if target['is_stopword']:
                continue
            if target['status'] in ('unknown', 'unknown_suggest'):
                continue

            # Get target's matches
            all_matches = target.get('all_matches', [])
            if not all_matches:
                continue

            # Split matches into city vs non-city
            city_matches = [
            ...
            # Unambiguous city (no non-city match) → always location
            if city_matches and not non_city_matches:
                if self.verbose:
                    print(f"  [{target['position']}] '{target['word']}' → "
                          f"CITY-ONLY (unambiguous)")
                continue

            # No city match at all → nothing to resolve
            if not city_matches:
                continue

            # ── AMBIGUOUS — inspect left context ──────────────────────
            ...
            for j in range(i - 1, -1, -1):
                left_wd = word_data[j]
                if left_wd['is_stopword']:
                    # Skip determiners/articles, stop at other stopwords
                    if left_wd['word'] in ('the', 'a', 'an', 'for', 'and'):
                        continue
                    break

                flags = set(left_wd.get('context_flags', []))
                left_flags.update(flags)

                pos = left_wd.get('pos') or left_wd.get('predicted_pos') or 'unknown'
                left_pos_types.append((pos, flags))

                if len(left_pos_types) >= 3:
                    break

            # ── Rule 1: Service/food/category on left → LOCATION ─────
            if left_flags & LOCATION_INTENT_FLAGS:
                if self.verbose:
                    print(f"  [{target['position']}] '{target['word']}' → "
                          f"LOCATION (left has: {left_flags & LOCATION_INTENT_FLAGS})")
                continue

            # ── Rule 2: Descriptor/adj + noun on left → KEYWORD ──────
            has_descriptor = bool(left_flags & KEYWORD_INTENT_FLAGS)
            has_adj = any(
                pos in ('adjective', 'adverb')
                for pos, _ in left_pos_types
            )
            has_noun = any(
                pos in ('noun', 'proper_noun')
                for pos, _ in left_pos_types
            )
            ...
            if has_descriptor and has_noun:
                should_demote = True
                reason_parts.append(f"descriptor+noun ({left_flags & KEYWORD_INTENT_FLAGS})")
            elif has_descriptor:
                should_demote = True
                reason_parts.append(f"descriptor ({left_flags & KEYWORD_INTENT_FLAGS})")
            elif has_adj and has_noun:
                should_demote = True
                reason_parts.append("adj+noun pattern")
            elif has_noun and not left_flags:
                # Bare noun + "in" + ambiguous → keyword
                # "women in media", "light in car"
                should_demote = True
                reason_parts.append("bare noun (no service flag)")

            if should_demote:
                # Select best non-city match
                non_city_matches.sort(key=lambda m: m.get('rank', 0), reverse=True)
                best = non_city_matches[0]

                target['selected_match'] = best
                target['pos'] = normalize_pos_string(best.get('pos', 'noun'))

                # Update context flags
                ctx = target.get('context_flags', [])
                ctx = [f for f in ctx if f != 'location_context']
                ctx.append('in_disambiguated_keyword')
                target['context_flags'] = ctx
                target['location_context'] = False

                resolved_any = True

                if self.verbose:
                    print(f"  [{target['position']}] '{target['word']}' → "
                          f"KEYWORD (demoted from city)")
                    print(f"       Reason: {', '.join(reason_parts)}")
                    print(f"       Was: {city_matches[0].get('category')} "
                          f"(rank={city_matches[0].get('rank', 0)})")
                    print(f"       Now: {best.get('category')} "
                          f"(rank={best.get('rank', 0)})")
                continue

            # ── Rule 3: No clear context → keep city (conservative) ──
            if self.verbose:
                print(f"  [{target['position']}] '{target['word']}' → "
                      f"CITY (conservative, no clear left context)")

        if not resolved_any and self.verbose:
            print("  (no ambiguous 'in' patterns found)")
...
# =========================================================================
    # STEP 4: Batch Correct Unknowns (Redis — only Redis step)
    #
    # REWRITE: Unknown words that get a confident fuzzy match are now
    # committed as 'corrected' so they flow into corrected_query and
    # get used for the actual Typesense search + re-embedding.
    #
    # Confidence gates (all must pass for auto-correct):
    #   - distance <= 1  (single character error)
    #   - suggestion rank >= 50  (not a garbage match)
    #   - POS compatible with predicted POS
    #
    # If the fuzzy match is distance 2 OR rank < 50 OR POS incompatible,
    # the word stays as 'unknown_suggest' (did-you-mean hint only,
    # original kept for search — same as old behavior).
    #
