...
#     if _analytics_client is None and ANALYTICS_AVAILABLE:
#         try:
#             _analytics_client = SearchAnalytics()
#         except Exception as e:
#             logger.warning(f"Failed to initialize SearchAnalytics: {e}")
...
#     # If no coordinates from params, use geolocation
#     if not user_location and location.get('lat') and location.get('lng'):
#         try:
#             user_location = (float(location['lat']), float(location['lng']))
#         except (TypeError, ValueError):
#             pass
...
#         'device_type': device_type,
#         'source': params.source,
#         # NEW: Add location to context for template use
...
#         if analytics:
#             try:
#                 session_id = data.get('session_id', '')[:50]
#                 if session_id:
...
#                 import traceback
#                 traceback.print_exc()
#                 return JsonResponse({'success': False, 'error': 'Event tracking failed'}, status=500)
...
#     clicked_url = data.get('url', '')[:2000]
    
#     if not session_id or not clicked_url:
#         return JsonResponse({'success': False, 'error': 'Missing required fields'}, status=400)
    
#     # Optional fields
#     clicked_position = 0
#     try:
#         clicked_position = int(data.get('position', 0))
#     except (TypeError, ValueError):
#         pass
    
#     result_id = data.get('result_id', '')[:100]
#     result_title = data.get('title', '')[:500]
#     result_source = data.get('source', '')[:200]
#     search_request_id = data.get('request_id', '')[:100]
...
#     corrected_query = data.get('corrected_query', '')[:500]
    
#     # Time to click
#     time_to_click_ms = None
#     try:
#         time_to_click_ms = int(data.get('time_to_click_ms', 0))
...
#             return JsonResponse({'success': False, 'error': 'Tracking failed'}, status=500)
    
#     return JsonResponse({'success': False, 'error': 'Analytics not available'}, status=503)
...
#     result_id = request.GET.get('result_id', '')[:100]
#     result_title = request.GET.get('title', '')[:500]
#     result_source = request.GET.get('source', '')[:200]
#     search_request_id = request.GET.get('request_id', '')[:100]
    
#     results_count = 0
#     try:
#         results_count = int(request.GET.get('results_count', 0))
#     except (TypeError, ValueError):
#         pass
    
#     was_corrected = request.GET.get('was_corrected', 'false').lower() == 'true'
#     corrected_query = request.GET.get('corrected_query', '')[:500]
