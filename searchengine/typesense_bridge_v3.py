...
#     Branches on document_data_type:
...
#     This rewards documents with both high ratings AND many reviews.
#     A 5-star business with 2 reviews scores lower than a 4.5-star
#     business with 300 reviews. The scale anchor sets the point at
#     which confidence reaches 1.0.
...
#         # Cap at 0.5 — quality signals are weaker than a real authority score
...
#     This is the fix for the questions pool pulling semantically
#     distant documents (distance 0.63–0.94).

#     Below the gate, score is linear relative to the gate:
#         score = 1.0 - (distance / gate)

#     So distance 0.0 → score 1.0
#        distance 0.325 → score 0.5
#        distance 0.65 → score 0.0
...
#     """
#     Return a multiplier based on how well the document's category
#     aligns with the query's detected domain.

#     Reads primary_domain from intent signals.
#     Checks document_category and document_schema against
#     DOMAIN_CATEGORY_MAP.

#     Returns:
#         1.15  — domain match (boost)
#         1.10  — partial match via schema
#         1.00  — neutral (no domain detected or no category on doc)
#         0.75  — domain mismatch (penalty)

#     Example:
#         primary_domain = 'food', doc category = 'food_recipes' → 1.15
#         primary_domain = 'food', doc category = 'technology'   → 0.75
#     """
#     primary_domain = signals.get('primary_domain')
#     if not primary_domain:
#         return 1.0

#     doc_category = (doc.get('document_category') or '').lower()
#     doc_schema   = (doc.get('document_schema') or '').lower()
...
#     local/shop   → want transactional or navigational documents
#     answer/explore → want informational or educational documents
#     browse       → want informational, commercial, or transactional
#     compare      → want informational or commercial

#     Returns:
#         1.10  — intent match (boost)
#         1.00  — neutral (no content_intent on doc or mode not mapped)
#         0.85  — intent mismatch (penalty)

#     Example:
#         query_mode = 'local', doc content_intent = 'transactional' → 1.10
#         query_mode = 'local', doc content_intent = 'informational' → 0.85
#     """
...
#     Example:
#         query_mode = 'local', doc type = 'business' → 1.0
#         query_mode = 'local', doc type = 'article'  → 0.5
#         query_mode = 'explore', doc type = 'article' → 1.0
#         query_mode = 'explore', doc type = 'product' → 0.5
#     """
#     data_type    = (doc.get('document_data_type') or '').lower()
...
#     5. domain_relevance     multiplier
#     6. content_intent_match multiplier
#     7. pool_type_multiplier multiplier
