...
def build_typesense_params(
    ...
    Field boost order (v5 — with subject hierarchy):
        primary_subject_name : 100  (highest — what the doc IS about)
        document_title       : 10
        entity_names         :  8
        primary_keywords     :  6
    ...
    field_boosts['primary_subject_name'] = 100
...
async def fetch_candidate_uuids(
    ...
    search_params = {
        ...
        'query_by_weights':      params.get('query_by_weights', '100,10,8,6,5,4,3'),
...
async def execute_full_search(
    ...
    if finished is not None:
        ...
        # DESYNC GUARD (same as keyword path): when no UI filter is active,
        # filtering must not drop anything. If it does, the cached docs lack the
        # filtered field. Page 2 routes through this FAST PATH, so the fix has to
        # live here too, or page 2 paginates a pool that disagrees with the count.
        ...
        if len(filtered_results) != len(all_results):
            ...
            print(f"   ⚠️ FAST PATH FILTER DESYNC: pool={len(all_results)} → "
        ...
        # Align the advertised count with the pool actually paginated.
