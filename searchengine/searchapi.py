...
from .vocabulary_cache import vocab_cache, ensure_loaded as ensure_cache_loaded


def get_from_local_cache(term: str) -> Optional[Dict[str, Any]]:
    """Try to get term from local cache first. Returns None if not found."""
    ...
    if len(words) == 3:
        data = vocab_cache.get_trigram(words[0], words[1], words[2])
        if data:
            return _format_cache_result(term_lower, data)
    
    # Check for bigram
    elif len(words) == 2:
        data = vocab_cache.get_bigram(words[0], words[1])
        if data:
            return _format_cache_result(term_lower, data)
    
    # Check for unigram
    else:
        data = vocab_cache.get_term(term_lower)
        if data:
            return _format_cache_result(term_lower, data)
...
def _format_cache_result(term: str, data: Dict) -> Dict[str, Any]:
    """Format local cache data to match Redis format."""
    return {
        'id': f"term:{term}:{data.get('category', 'unknown').lower().replace(' ', '_')}",
        'member': f"term:{term}",
        'term': data.get('term', term),
        'display': data.get('display', term),
        'description': data.get('description', ''),
        'category': data.get('category', ''),
        'entity_type': data.get('entity_type', 'unigram'),
...
def classify_query_local(query: str) -> Optional[Dict[str, Any]]:
    """Use local cache to classify entire query at once."""
    if not vocab_cache.loaded:
        ensure_cache_loaded()
...
def get_term_metadata(member: str) -> Optional[Dict[str, Any]]:
    ...
    PRIORITY: Local cache first, then Redis.
    """
    if not member:
        return None
    ...
    base_term = extract_base_term(member)
    local_result = get_from_local_cache(base_term)
    if local_result:
        return local_result
...
def get_exact_term_matches(term: str) -> List[Dict[str, Any]]:
    ...
    PRIORITY:
    1. Local vocabulary cache (instant, ~0.001ms)
    2. Redis (fallback, ~300ms)
    """
    if not term:
        return []
    ...
    # === TRY LOCAL CACHE FIRST ===
    local_result = get_from_local_cache(term_lower)
    if local_result:
        return [local_result]
...
def get_prefix_matches(prefix: str, limit: int = 50) -> List[Dict[str, Any]]:
    """
    Get terms that start with the given prefix.
    PRIORITY: Local cache first (if available), then Redis.
    """
    if not prefix:
        return []
    ...
    if vocab_cache.loaded:
        # Search through local cache terms that start with prefix
        for term, metadata in vocab_cache.terms.items():
            if term.startswith(prefix_lower):
                if term not in seen_terms:
                    formatted = _format_cache_result(term, metadata)
        ...
        # Also check bigrams
        for bigram, metadata in vocab_cache.bigrams.items():
            if bigram.startswith(prefix_lower):
                if bigram not in seen_terms:
                    formatted = _format_cache_result(bigram, metadata)
                    matches.append(formatted)
    ...
    try:
        ...
        for doc in result.docs:
            parsed = parse_search_doc(doc)
            if parsed:
                term_lower = parsed.get('term', '').lower()
                if term_lower not in seen_terms:
                    matches.append(parsed)
                    seen_terms.add(term_lower)
...
def get_fuzzy_matches(term: str, limit: int = 10, max_distance: int = 2) -> List[Dict[str, Any]]:
    ...
    Note: Fuzzy search requires Redis - local cache doesn't support fuzzy matching.
    """
    client = RedisLookupTable.get_client()
    if not client or not term:
        return []
...
def batch_get_term_metadata(members: List[str]) -> Dict[str, Dict[str, Any]]:
    """
    Batch get metadata for multiple terms.
    PRIORITY: Local cache first, then Redis for remaining.
    """
    if not members:
        return {}
    
    metadata_dict = {}
    members_needing_redis = []
    
    # === FIRST PASS: LOCAL CACHE ===
    for member in members:
        base_term = extract_base_term(member)
        local_result = get_from_local_cache(base_term)
        if local_result:
            metadata_dict[member] = local_result
        else:
            members_needing_redis.append(member)
    
    # === SECOND PASS: REDIS FALLBACK ===
    if not members_needing_redis:
        return metadata_dict
    ...
    try:
        ...
        for member in members_needing_redis:
            hash_key = member if member.startswith('term:') else f'term:{member}'
            pipeline.hgetall(hash_key)
        
        results = pipeline.execute()
        
        for member, metadata in zip(members_needing_redis, results):
            if metadata:
                base_term = extract_base_term(member)
                try:
                    rank_val = int(float(metadata.get('rank', 0)))
...
def batch_validate_words_redis(words: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    PRIORITY: Local cache first, Redis as fallback.
    """
    if not words:
        return {}
    ...
    for word in words:
        ...
        local_result = get_from_local_cache(word_lower)
        if local_result:
            results[word_lower] = {
                'is_valid': True,
                'word': word_lower,
                'member': local_result.get('id', ''),
                'matches': [local_result],
                'metadata': local_result,
                'source': 'local_cache'
            }
        else:
            words_needing_redis.append(word_lower)
    
    # === SECOND PASS: REDIS FALLBACK ===
    if words_needing_redis:
        client = RedisLookupTable.get_client()
        if client:
            for word_lower in words_needing_redis:
                # Use Redis direct lookup
                term_key = word_lower.replace(' ', '_')
                pattern = f"term:{term_key}:*"
                ...
                try:
                    ...
                    print(f"Redis lookup error for {word_lower}: {e}")
                    results[word_lower] = {
                        'is_valid': False,
                        'word': word_lower,
                        'member': None,
            ...
            # Redis unavailable - mark remaining as not found
            for word_lower in words_needing_redis:
                results[word_lower] = {
                    'is_valid': False,
                    'word': word_lower,
...
def validate_word(
    ...
    # Check for exact match (local cache first, then Redis)
    exact_matches = get_exact_term_matches(word_lower)
    ...
    # Word not found - get fuzzy suggestion (Redis only)
    result = get_suggestions(word_lower, limit=1, max_distance=2)
...
def get_suggestions(
    ...
    PRIORITY: Local cache for exact/prefix, Redis for fuzzy.
    """
    response = {
        'success': True,
        'input': input_text,
    ...
    try:
        ...
        # === TIER 1: EXACT MATCH (local cache first) ===
        exact_matches = get_exact_term_matches(input_lower)
        ...
        # === TIER 2: PREFIX MATCH (local cache first) ===
        prefix_results = get_prefix_matches(input_lower, limit=limit * 3)
        ...
        # === TIER 3: FUZZY MATCH (Redis only, if we have few results) ===
        if len(all_results) < limit:
            fuzzy_results = get_fuzzy_matches(input_lower, limit=limit * 2, max_distance=max_distance)
...
def get_top_words_by_rank(limit: int = 200) -> List[Dict[str, Any]]:
    ...
    if vocab_cache.loaded:
        ...
        all_terms.sort(key=lambda x: x.get('rank', 0), reverse=True)
...
def search_by_category(
    ...
    if vocab_cache.loaded:
        for term, metadata in vocab_cache.terms.items():
            if term.startswith(query_lower):
                term_category = metadata.get('category', '').lower().replace(' ', '_')
                if term_category == category_lower:
                    if term not in seen_terms:
                        formatted = _format_cache_result(term, metadata)
                        matches.append(formatted)
                        seen_terms.add(term)
    ...
    try:
        escaped_query = escape_query(query_lower)
        escaped_category = escape_tag(category)
...
def search_by_entity_type(
    ...
    # Try local cache first
    if vocab_cache.loaded:
        # Choose the right dictionary based on entity type
        if entity_type_lower == 'bigram':
            source_dict = vocab_cache.bigrams
        elif entity_type_lower == 'trigram':
            source_dict = vocab_cache.trigrams
        else:
            source_dict = vocab_cache.terms
        
        for term, metadata in source_dict.items():
            if term.startswith(query_lower):
                if term not in seen_terms:
                    formatted = _format_cache_result(term, metadata)
                    matches.append(formatted)
...
def lookup_table(
    ...
    """
    Main API endpoint function for search preprocessing.
    PRIORITY: Local vocabulary cache first, Redis as fallback.
    """
    response = {
        'success': True,
        'query': query,
    ...
    try:
        ...
        # Check Redis cache for previously processed queries
        if check_cache_first:
            cached = check_cache(query)
            if cached:
                response['cache_hit'] = True
        ...
        # Batch validate all words (local cache first, then Redis)
        validation_cache = batch_validate_words_redis(words)
...
def get_lookup_status() -> Dict[str, Any]:
    """Get status of both local cache and Redis connection."""
    status = {
        'local_cache': {
            'loaded': vocab_cache.loaded,
            'term_count': vocab_cache.term_count,
            'cities': len(vocab_cache.cities),
            'states': len(vocab_cache.states),
            'locations': len(vocab_cache.locations),
            'bigrams': len(vocab_cache.bigrams),
            'trigrams': len(vocab_cache.trigrams),
            'load_source': vocab_cache.load_source,
            'last_updated': vocab_cache.last_updated,
        },
        'redis': {
            'connected': False,
            'error': None
