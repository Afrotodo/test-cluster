...
class SearchAnalytics:
    ...
    def start_session(
        ...
        Args:
            ...
            user_id: User ID if logged in, None if anonymous
            device_type: 'desktop', 'mobile', 'tablet'
            user_agent: Browser user agent string
            ip_address: User's IP (for geo lookup)
            location: Pre-resolved location dict {city, region, country, lat, lng}
            referrer: How they got to the site
        ...
        if existing:
            ...
            if location and location.get('city'):
                self.redis.zincrby("analytics:locations:cities", 1, f"{location.get('city')}, {location.get('country', '')}")
    ...
    def _get_time_bucket(self, ms: int) -> str:
        """Bucket time-to-click for distribution analysis."""
        if ms < 1000:
            return 'under_1s'
        elif ms < 3000:
            return '1s_to_3s'
    ...
    def track_event(
        ...
        session_events_key = f"session:{session_id}:events"
        self.redis.lpush(session_events_key, json.dumps(event))
        self.redis.ltrim(session_events_key, 0, 999)
        self.redis.expire(session_events_key, self.session_expire_seconds)
        
        # Track event type counts
        date_key = datetime.utcnow().strftime('%Y-%m-%d')
        self.redis.hincrby(f"analytics:events:{date_key}", event_type, 1)
        self.redis.expire(f"analytics:events:{date_key}", ANALYTICS_RETENTION_DAYS * 86400)
