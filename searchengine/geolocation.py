...
ABSTRACTAPI_KEY = config('ABSTRACTAPI_GEOLOCATION_KEY', default='')
ABSTRACTAPI_URL = config('ABSTRACTAPI_GEOLOCATION_URL', default='https://ipgeolocation.abstractapi.com/v1/')
...
def _get_location_from_api_internal(ip_address: str) -> Dict[str, Any]:
    ...
    if not ABSTRACTAPI_KEY:
        logger.warning("ABSTRACTAPI_GEOLOCATION_KEY not configured in environment")
        return {}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error("AbstractAPI authentication failed - check ABSTRACTAPI_GEOLOCATION_KEY")
            return {}
...
def health_check() -> Dict[str, Any]:
    ...
    # Check API key
    if not ABSTRACTAPI_KEY:
        result['errors'].append('ABSTRACTAPI_GEOLOCATION_KEY not configured')
